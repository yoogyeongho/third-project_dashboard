...
# # 데이터 파일 경로를 정확하게 지정합니다..
current_dir = os.path.dirname(os.path.abspath(__file__))

# 데이터 파일 경로를 동적으로 구성합니다.(파일 경로 찾기 실패로 인해 보완)
data_path1 = os.path.join(current_dir, '../../앙극산화피막_매칭데이터셋.csv')
data_path2 = os.path.join(current_dir, '../../failure_df.csv')
try:
    
    df = pd.read_csv('../../앙극산화피막_매칭데이터셋.csv', sep=',')
